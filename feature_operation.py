...
class FeatureOperator:
    ...
    @staticmethod
    def tally_job(args):
        features, data, threshold, tally_labels, tally_units, tally_units_cat, tally_both, start, end, vis, num = args
        ...
        for batch in pd.batches():
            ...
            for concept_map in batch:
                ...
                for unit_id in range(units):
                    ...
                    if vis:
                        if (unit_id == num and img_index % 100 == 0):
                            image = imageio.imread((data.filename(img_index)))
                            activation_map = cv2.resize(src = feature_map, dsize = image.shape[:2])
                            visualization = (activation_map[:,:,numpy.newaxis] * 0.8 + 0.2) *image
                            imsize = 224
                            if visualization.shape[:2] != (imsize, imsize):
                                visualization = cv2.resize(src = visualization, dsize=(imsize, imsize))
                            imageio.imwrite('./activation/unit_id_%d/%d.jpeg' % (num,img_index), visualization)    
                    
                    if feature_map.max() > threshold[unit_id]:
                        mask = cv2.resize(src = feature_map, dsize = (concept_map['sh'], concept_map['sw']))
    ...
    def tally(self, features, threshold, savepath='', vis=False, num=0):
        ...
        if settings.PARALLEL > 1:
            ...
            ranges = [(s, min(self.data.size(), s + psize)) for s in range(0, self.data.size(), psize) if
                      s < self.data.size()]
            params = [(features, self.data, threshold, tally_labels, tally_units, tally_units_cat, tally_both) + r for r in ranges + vis + num]
            threadpool = pool.ThreadPool(processes=settings.PARALLEL)
            threadpool.map(FeatureOperator.tally_job, params)
        else:
            FeatureOperator.tally_job((features, self.data, threshold, tally_labels, tally_units, tally_units_cat, tally_both, 0, self.data.size(), vis, num))
